...
Graphical Abstract Creator는 EatRice Lab 작품입니다. 편집 가능한 벡터 PowerPoint 그래픽 초록을 생성합니다. 최소 입력: 그래픽 초록에 표시할 내용을 설명하세요.

## Codex에 설치

`skill.zip`을 준비하고 Codex 또는 ChatGPT의 Skills 관리 화면에서 업로드한 뒤 `graphical-abstract-creator`를 활성화합니다. 업데이트할 때는 새 `skill.zip`으로 기존 버전을 교체합니다.
